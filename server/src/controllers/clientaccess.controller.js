...
const shareAccess = async (req, res) => {
    try {
        ...
        if (permissions && (permissions.includes('update') || permissions.includes('delete')) && !permissions.includes('read')) {
...
const getSharedWithMe = async (req, res) => {
    try {
        ...
        const sharedWithMe = await ClientAccess.find({ sharedWithId: req.user.userId })
            .populate('ownerId', 'name email')
            .sort({ createdAt: -1 });
...
const updateSharingPermissions = async (req, res) => {
    try {
        ...
        // If 'update' or 'delete' is present, ensure 'read' is also included
        if ((permissions.includes('update') || permissions.includes('delete')) && !permissions.includes('read')) {
            permissions.push('read');
        }
